...
\midrule
\texttt{directory} & Output directory name & "cosmos\_output" \\
...
\midrule
\texttt{thermo} & Use temperature-based random vectors (Boltzmann distribution) \\
\texttt{atomic} & Use energy-weighted random vectors based on per-atom energies \\
\texttt{nl} & Combine base random with local rigid movement (Nl) \\
\texttt{element} & Use element-weighted random vectors \\
\texttt{python} & Use custom user-defined function \\
\bottomrule
\end{tabular}
...
def generate_random_direction(atoms: Atoms) -> np.ndarray:
    """
    Generate custom random direction vector for CoSMoS algorithm.
